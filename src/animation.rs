...
pub fn animate_explosion_shockwave(
    ...
    for (entity, mut shockwave, mut transform, mut sprite) in query.iter_mut() {
        ...
        let current_radius =
            shockwave.start_radius + (shockwave.max_radius - shockwave.start_radius) * progress;
        ...
        sprite.color = Color::srgba(brightness * 1.0, brightness * 0.6, brightness * 0.1, alpha);
